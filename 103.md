...
3. Client chooses a provider(s) and POSTs to their `endpoint`. 
  - The POST body should match exactly what is required by the underlying API less the API key. ( Or following the note's `schema` )
4. The service provider should validate their request and return a lightning invoice with a `successAction` url. 
  - The `successAction` url should be formatted as such: `<endpoint>/<invoice_hash>/get_result`
5. The client pays.
